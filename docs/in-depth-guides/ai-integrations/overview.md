...
- **[Building an agent using Microsoft Agent Framework in Teams](./build-agent-microsoft-agent-framework.md)** - creates the core agent and basic Teams integration, adds local and remote MCP tools, streams responses into Teams, and maintains per-conversation memory using sessions.
